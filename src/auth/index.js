    ...
    async created() {
      ...
      try {
        ...
        if(this.isAuthenticated){
          axios.get("https://tpftestbackend.herokuapp.com/usuarios")
            .then(res => {
                var encontre = false;
                for(var user in res.data){
                  if (this.user.email ==  res.data[user].email)
                    encontre = true;
                }
                if(!encontre) {
                  const headers = {
                  ...
                  var usuario = { email: this.user.email, nombre: this.user.name, telefono: "", admin: false }
